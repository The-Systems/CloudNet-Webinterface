...
if($pagestyle == 2) {
	if(isset($_SESSION['Logged'])) {
		...
		$server = $_GET['server'];
		$sendcommand = "cmd $server $command";
		...
		header('Location: '.$domain.'/logged/index.php?erfolg=commandsendtoserver&#4');
        //erfolg
    } else {
		header('Location: '.$domain.'/index.php?error=403');
//error
	...
	if(isset($_SESSION['Logged'])) {
		...
		$jsonlol = file_get_contents($url, false, $context);
		$json = json_decode($jsonlol);
		header('Location: '.$domain.'/logged/index.php?erfolg=commandsendtoserver&#7');
        //erfolg
    } else {
		header('Location: '.$domain.'/index.php?error=403');
//error
