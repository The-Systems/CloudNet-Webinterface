...
if($pagestyle == 2) {
	if(isset($_SESSION['Logged'])) {
		$gruppe = $_GET['gruppe'];
		$anzahl = $_GET['anzahl'];
		$sendcommand = "create PROXY $gruppe $anzahl";
		...
		header('Location: '.$domain.'/logged/index.php?set=proxy&erfolg=proxystart&#1');
        //erfolg
    } else {
		header('Location: '.$domain.'/index.php?error=403');
//error
	...
	if(isset($_SESSION['Logged'])) {
		...
		$jsonlol = file_get_contents($url, false, $context);
		$json = json_decode($jsonlol);
		header('Location: '.$domain.'/logged/index.php?erfolg=proxystart&#4');
        //erfolg
    } else {
		header('Location: '.$domain.'/index.php?error=403');
//error
